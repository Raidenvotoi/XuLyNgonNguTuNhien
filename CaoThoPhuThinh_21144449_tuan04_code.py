...
def crawl_text(limit):
    ...
    token="eyJhbGciOiJSUzI1NiIsImtpZCI6IlNIQTI1NjpzS3dsMnlsV0VtMjVmcXhwTU40cWY4MXE2OWFFdWFyMnpLMUdhVGxjdWNZIiwidHlwIjoiSldUIn0..YEyQzAuXMQ56sNf09l39LuprZluX_QrHgZinOPT7M1ZbSV19yNAyacuD3Edsir1FcVwJQbhlWq6mDqDsAATGCicJU4WpkKk0aC7JQ8wAhZDjjj56XOy7XN06aWxACY6pxPQcQ5yRsVs8j1MjbWuD5_Wgk9Wt6ARFcVCrL_ht48MDpRAPFcliZ5P6CpQEkrXfhc954aiUhdx1gKt0ZrWrL7LYdSEFBCQM0oMJuCZrrpQUpxP9IUwpqzwrJYjuqMjtgTKgx2XhS1zXCARkh79PYjXCA6IbEGq4ENeoD5lnBQbETo-5hJz3KwNew9wkGS_oGE4FVLP-ng-QFfa2x5_EaA"
    headers = {
...
st.title("Bước 3 làm sạch dữ liệu")
...
st.title("Bước 4 Xử lý tiền dữ liệu ")
...
if option_pre =="One-Hot Encoding"  or option_pre =="Bag of Words (BoW)" :
    selected_text = st.selectbox("Chọn bài để phân tích:", st.session_state.df2["text"])
    if  str(selected_text) !="" :
        doc = nlp(str(selected_text))
        sentences = list(doc.sents)
if st.button("Xử lý bieu dien"):
